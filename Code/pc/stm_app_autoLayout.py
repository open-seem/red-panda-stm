# stm_app.py
# 在stm_app.py的基础上修改了布局方式，使得界面能够自动适应不同分辨率和缩放的屏幕
...
class App(tk.Tk):
    def __init__(self):
        ...
        # 3. 动态计算绘图区域和每个图窗的尺寸
        control_panel_width = 380  # 为左侧控制面板保留一个固定的宽度
        # 计算右侧绘图区域的可用空间
        plot_area_width = screen_width - control_panel_width
        plot_area_height = screen_height
        
        # 为3列2行的图窗布局计算每个图窗的尺寸
        plot_width = (plot_area_width + 100) / 3 
        plot_height = (plot_area_height + 100) / 2

        # 确保计算出的尺寸不为负数（在屏幕极小的情况下）
        ...
        ### --- 更改 --- ###
        # self.baseline_size = 700  # 已被上面的动态计算取代，删除此行
        
        # 使用动态计算的尺寸来创建框架
        self.image_frame = ttk.Frame(
            self, width=plot_area_width, height=plot_area_height)
        self.image_frame.grid(row=0, column=1, padx=10, pady=5)

        # Image frame settings
        ### --- 更改开始 --- ###
        # 在创建 PlotFrame 时使用计算出的 plot_width 和 plot_height
        self.real_time_current_plot_frame = PlotFrame(
            self.image_frame, dpi=100.0, with_toobar=True, width=plot_width, height=plot_height)
        ### --- 更改结束 --- ###
        ...
        self.control_frames = ttk.Frame(
            self, width=control_panel_width, height=plot_area_height) # 使用动态高度
        ### --- 更改结束 --- ###
        ...
        ### --- 更改 --- ###
        # self.state('zoomed') # 已被 geometry() 取代
        self.state('zoomed') # 经过测试，在Windows上zoomed效果更好，它能正确处理任务栏。在Linux/Mac上geometry可能更好。此处保留zoomed。
    def _quit(self):
    ...
    def _update_real_time(self):
        if not self.stm.busy:
            ...
            if self.stm.history: ### 更改：增加一个检查，防止 history 为空时出错
                plot_x = [hist.time_millis for hist in self.stm.history]
                self.status_label.config(text=status.to_string())
                max_time = max(plot_x)
                # 这个时间轴计算方式可能会导致横轴显示不直观，暂时保留原样
                plot_x = [(x - max_time) / 700 * 2.0 for x in plot_x]
    ...
    def _plot_iv_curve(self, *args):
        # 将参数转换为整数，因为它们来自输入框的字符串
        int_args = [int(arg) for arg in args]
        iv_curve_values = self.stm.measure_iv_curve(*int_args)
        x_value = iv_curve_values[::2]
        y_value = iv_curve_values[1::2]
